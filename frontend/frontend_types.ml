...
type expr_p_null_lit = {
  null_texpr : type_expr;
}
...
let rec default_expr_p_null_lit 
  ?null_texpr:((null_texpr:type_expr) = default_type_expr ())
  () : expr_p_null_lit  = {
  null_texpr;
}
...
and default_expr_p_binop 
  ...
  ?op_line_no:((op_line_no:int32) = 0l)
  () : expr_p_binop  = {
  bin_op;
  lexpr;
  rexpr;
...
and default_expr_p_array_creation 
  ...
  ?arr_cons_idx:((arr_cons_idx:int32) = 0l)
...
and default_expr_p_cast_expr 
  ...
  ?cast_line_no:((cast_line_no:int32) = 0l)
  () : expr_p_cast_expr  = {
  cast_to;
  expr;
  cast_type;
...
and default_expr_p_class_creation 
  ?con_texpr:((con_texpr:type_expr) = default_type_expr ())
  ?con_args:((con_args:expr list) = [])
  ?vtable_index:((vtable_index:int32) = 0l)
...
and default_expr_p_method_call 
  ?method_idx:((method_idx:int32) = 0l)
  ?obj_expr:((obj_expr:expr) = default_expr ())
  ?method_args:((method_args:expr list) = [])
  () : expr_p_method_call  = {
  method_idx;
  obj_expr;
  method_args;
...
let rec default_stmt_p_delete 
  ?del_expr:((del_expr:expr) = default_expr ())
  () : stmt_p_delete  = {
  del_expr;
}

let rec default_stmt_p_free 
  ?free_expr:((free_expr:expr) = default_expr ())
  () : stmt_p_free  = {
  free_expr;
}

let rec default_stmt_p_return 
  ?ret_expr:((ret_expr:expr) = default_expr ())
  () : stmt_p_return  = {
  ret_expr;
}
