...
and default_expr_p_if_expr 
  ...
  ?if_expr:((if_expr:expr) = default_expr ())
  ?else_expr:((else_expr:expr) = default_expr ())
  () : expr_p_if_expr  = {
  eval;
  if_expr;
  else_expr;
...
and default_expr_p_block 
  ?expr_list:((expr_list:expr list) = [])
  () : expr_p_block  = {
  expr_list;
}
...
and default_expr_p_while_expr 
  ...
  ?while_block:((while_block:expr) = default_expr ())
  () : expr_p_while_expr  = {
  while_cond;
  while_block;
}
