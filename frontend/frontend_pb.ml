...
type expr_p_null_lit_mutable = {
  mutable null_texpr : Frontend_types.type_expr;
}
...
let default_expr_p_method_call_mutable () : expr_p_method_call_mutable = {
  method_idx = 0l;
  obj_expr = Frontend_types.default_expr ();
  method_args = [];
}
...
type stmt_p_return_mutable = {
  mutable ret_expr : Frontend_types.expr;
}
...
let rec decode_expr_p_null_lit d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.null_texpr <- decode_type_expr (Pbrt.Decoder.nested d); null_texpr_is_set := true;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(expr_p_null_lit), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  begin if not !null_texpr_is_set then Pbrt.Decoder.missing_field "null_texpr" end;
  ({
    Frontend_types.null_texpr = v.null_texpr;
  } : Frontend_types.expr_p_null_lit)
...
and decode_expr d = 
  let rec loop () = 
    let ret:Frontend_types.expr = match Pbrt.Decoder.key d with
      ...
      | Some (10, _) -> (Frontend_types.Null_lit (decode_expr_p_null_lit (Pbrt.Decoder.nested d)) : Frontend_types.expr) 
      ...
      | Some (13, _) -> (Frontend_types.Method_call (decode_expr_p_method_call (Pbrt.Decoder.nested d)) : Frontend_types.expr) 
      | Some (n, payload_kind) -> (
        Pbrt.Decoder.skip d payload_kind; 
        loop () 
      )
...
and decode_expr_p_array_creation d =
  ...
  while !continue__ do
    ...
    | Some (4, Pbrt.Varint) -> begin
      v.arr_cons_idx <- Pbrt.Decoder.int32_as_varint d; arr_cons_idx_is_set := true;
    end
    | Some (4, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(expr_p_array_creation), field(4)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  begin if not !arr_cons_idx_is_set then Pbrt.Decoder.missing_field "arr_cons_idx" end;
...
and decode_expr_p_method_call d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Varint) -> begin
      v.method_idx <- Pbrt.Decoder.int32_as_varint d; method_idx_is_set := true;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(expr_p_method_call), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.obj_expr <- decode_expr (Pbrt.Decoder.nested d); obj_expr_is_set := true;
    end
    | Some (2, pk) -> 
  ...
  begin if not !obj_expr_is_set then Pbrt.Decoder.missing_field "obj_expr" end;
  begin if not !method_idx_is_set then Pbrt.Decoder.missing_field "method_idx" end;
  ({
...
let rec decode_stmt_p_return d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.ret_expr <- decode_expr (Pbrt.Decoder.nested d); ret_expr_is_set := true;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(stmt_p_return), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  begin if not !ret_expr_is_set then Pbrt.Decoder.missing_field "ret_expr" end;
  ({
    Frontend_types.ret_expr = v.ret_expr;
  } : Frontend_types.stmt_p_return)
...
and decode_stmt d = 
  let rec loop () = 
    let ret:Frontend_types.stmt = match Pbrt.Decoder.key d with
      ...
      | Some (11, _) -> (Frontend_types.Return (decode_stmt_p_return (Pbrt.Decoder.nested d)) : Frontend_types.stmt) 
      | Some (n, payload_kind) -> (
        Pbrt.Decoder.skip d payload_kind; 
        loop () 
      )
