...
type var_p_simple_mutable = { mutable var_name : string }

let default_var_p_simple_mutable () : var_p_simple_mutable = { var_name = "" }

type type_expr_p_class_mutable = { mutable name : string }

let default_type_expr_p_class_mutable () : type_expr_p_class_mutable = { name = "" }

type type_expr_p_pointer_mutable = { mutable data : Frontend_types.type_expr }

let default_type_expr_p_pointer_mutable () : type_expr_p_pointer_mutable =
  { data = Frontend_types.default_type_expr () }
;;

type expr_p_null_lit_mutable = { mutable null_texpr : Frontend_types.type_expr }

let default_expr_p_null_lit_mutable () : expr_p_null_lit_mutable =
  { null_texpr = Frontend_types.default_type_expr () }
;;
...
let default_var_p_field_mutable () : var_p_field_mutable =
  { base_expr = Frontend_types.default_expr (); field_index = 0l; field_line_no = 0l }
;;
...
let default_expr_p_assign_mutable () : expr_p_assign_mutable =
  { lhs = Frontend_types.default_var (); rhs = Frontend_types.default_expr () }
;;

type identifier_mutable = { mutable id : Frontend_types.var }
...
type stmt_p_expr_stmt_mutable = { mutable expr_stmt : Frontend_types.expr }
...
type stmt_p_return_mutable = { mutable ret_expr : Frontend_types.expr }

let default_stmt_p_return_mutable () : stmt_p_return_mutable =
  { ret_expr = Frontend_types.default_expr () }
;;
...
type stmt_p_block_mutable = { mutable stmt_list : Frontend_types.stmt list }

let default_stmt_p_block_mutable () : stmt_p_block_mutable = { stmt_list = [] }
...
let default_param_mutable () : param_mutable =
  { param_type = Frontend_types.default_type_expr (); param_name = "" }
;;
...
let rec decode_var_p_simple d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(var_p_simple), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !var_name_is_set then Pbrt.Decoder.missing_field "var_name";
  ({ Frontend_types.var_name = v.var_name } : Frontend_types.var_p_simple)
;;
...
let rec decode_type_expr_p_class d =
  ...
  ({ Frontend_types.name = v.name } : Frontend_types.type_expr_p_class)
;;
...
let rec decode_type_expr d =
  let rec loop () =
    let ret : Frontend_types.type_expr =
      ...
      | Some (2, _) ->
        (Frontend_types.Class (decode_type_expr_p_class (Pbrt.Decoder.nested d))
          : Frontend_types.type_expr)
      | Some (3, _) ->
        (Frontend_types.Pointer (decode_type_expr_p_pointer (Pbrt.Decoder.nested d))
          : Frontend_types.type_expr)
      | Some (4, _) ->
        Pbrt.Decoder.empty_nested d;
...
and decode_type_expr_p_pointer d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.data <- decode_type_expr (Pbrt.Decoder.nested d);
      data_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(type_expr_p_pointer), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !data_is_set then Pbrt.Decoder.missing_field "data";
  ({ Frontend_types.data = v.data } : Frontend_types.type_expr_p_pointer)
;;

let rec decode_expr_p_null_lit d =
  let v = default_expr_p_null_lit_mutable () in
  let continue__ = ref true in
  let null_texpr_is_set = ref false in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.null_texpr <- decode_type_expr (Pbrt.Decoder.nested d);
      null_texpr_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(expr_p_null_lit), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !null_texpr_is_set then Pbrt.Decoder.missing_field "null_texpr";
  ({ Frontend_types.null_texpr = v.null_texpr } : Frontend_types.expr_p_null_lit)
;;

let rec decode_expr_p_cast d =
  let rec loop () =
    let ret : Frontend_types.expr_p_cast =
      match Pbrt.Decoder.key d with
...
let rec decode_var_p_subscript d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.base_var <- decode_var (Pbrt.Decoder.nested d);
      base_var_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(var_p_subscript), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.var_exp <- decode_expr (Pbrt.Decoder.nested d);
      var_exp_is_set := true
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(var_p_subscript), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
      v.len_var <- decode_var (Pbrt.Decoder.nested d);
      len_var_is_set := true
    | Some (3, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(var_p_subscript), field(3)" pk
    | Some (4, Pbrt.Varint) ->
      v.line_no <- Pbrt.Decoder.int32_as_varint d;
      line_no_is_set := true
    | Some (4, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(var_p_subscript), field(4)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !line_no_is_set then Pbrt.Decoder.missing_field "line_no";
  if not !len_var_is_set then Pbrt.Decoder.missing_field "len_var";
  if not !var_exp_is_set then Pbrt.Decoder.missing_field "var_exp";
  if not !base_var_is_set then Pbrt.Decoder.missing_field "base_var";
...
and decode_var d =
  let rec loop () =
    let ret : Frontend_types.var =
      match Pbrt.Decoder.key d with
      | None -> Pbrt.Decoder.malformed_variant "var"
      | Some (1, _) ->
        (Frontend_types.Simple (decode_var_p_simple (Pbrt.Decoder.nested d))
          : Frontend_types.var)
      | Some (2, _) ->
        (Frontend_types.Subscript (decode_var_p_subscript (Pbrt.Decoder.nested d))
          : Frontend_types.var)
      | Some (3, _) ->
        (Frontend_types.Field (decode_var_p_field (Pbrt.Decoder.nested d))
          : Frontend_types.var)
      | Some (4, _) ->
        (Frontend_types.Load_var (decode_var_p_load (Pbrt.Decoder.nested d))
          : Frontend_types.var)
      | Some (n, payload_kind) ->
        Pbrt.Decoder.skip d payload_kind;
        loop ()
...
and decode_var_p_field d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.base_expr <- decode_expr (Pbrt.Decoder.nested d);
      base_expr_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(var_p_field), field(1)" pk
    | Some (2, Pbrt.Varint) ->
      v.field_index <- Pbrt.Decoder.int32_as_varint d;
      field_index_is_set := true
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(var_p_field), field(2)" pk
    | Some (3, Pbrt.Varint) ->
      v.field_line_no <- Pbrt.Decoder.int32_as_varint d;
      field_line_no_is_set := true
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(var_p_field), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !field_line_no_is_set then Pbrt.Decoder.missing_field "field_line_no";
  if not !field_index_is_set then Pbrt.Decoder.missing_field "field_index";
  if not !base_expr_is_set then Pbrt.Decoder.missing_field "base_expr";
...
and decode_expr d =
  let rec loop () =
    let ret : Frontend_types.expr =
      ...
      | Some (8, _) ->
        (Frontend_types.Array_creation
           (decode_expr_p_array_creation (Pbrt.Decoder.nested d))
          : Frontend_types.expr)
      | Some (9, _) ->
        (Frontend_types.Var_exp (decode_var (Pbrt.Decoder.nested d))
          : Frontend_types.expr)
      | Some (10, _) ->
        (Frontend_types.Null_lit (decode_expr_p_null_lit (Pbrt.Decoder.nested d))
          : Frontend_types.expr)
      | Some (11, _) ->
...
and decode_expr_p_assign d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.lhs <- decode_var (Pbrt.Decoder.nested d);
      lhs_is_set := true
    | Some (1, pk) ->
...
and decode_identifier d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.id <- decode_var (Pbrt.Decoder.nested d);
      id_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(identifier), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !id_is_set then Pbrt.Decoder.missing_field "id";
  ({ Frontend_types.id = v.id } : Frontend_types.identifier)
...
and decode_expr_p_array_creation d =
  ...
  while !continue__ do
    ...
    | Some (3, Pbrt.Varint) ->
      v.make_line_no <- Pbrt.Decoder.int32_as_varint d;
...
and decode_expr_p_cast_expr d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.cast_to <- decode_type_expr (Pbrt.Decoder.nested d);
      cast_to_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(expr_p_cast_expr), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.expr <- decode_expr (Pbrt.Decoder.nested d);
      expr_is_set := true
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(expr_p_cast_expr), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
      v.cast_type <- decode_expr_p_cast (Pbrt.Decoder.nested d);
      cast_type_is_set := true
    | Some (3, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(expr_p_cast_expr), field(3)" pk
    | Some (4, Pbrt.Varint) ->
      v.cast_line_no <- Pbrt.Decoder.int32_as_varint d;
      cast_line_no_is_set := true
    | Some (4, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(expr_p_cast_expr), field(4)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !cast_line_no_is_set then Pbrt.Decoder.missing_field "cast_line_no";
  if not !cast_type_is_set then Pbrt.Decoder.missing_field "cast_type";
  if not !expr_is_set then Pbrt.Decoder.missing_field "expr";
  if not !cast_to_is_set then Pbrt.Decoder.missing_field "cast_to";
...
and decode_var_p_load d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.var <- decode_var (Pbrt.Decoder.nested d);
      var_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(var_p_load), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !var_is_set then Pbrt.Decoder.missing_field "var";
  ({ Frontend_types.var = v.var } : Frontend_types.var_p_load)
;;
...
let rec decode_stmt_p_expr_stmt d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.expr_stmt <- decode_expr (Pbrt.Decoder.nested d);
      expr_stmt_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(stmt_p_expr_stmt), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !expr_stmt_is_set then Pbrt.Decoder.missing_field "expr_stmt";
...
let rec decode_stmt_p_return d =
  let v = default_stmt_p_return_mutable () in
  let continue__ = ref true in
  let ret_expr_is_set = ref false in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.ret_expr <- decode_expr (Pbrt.Decoder.nested d);
      ret_expr_is_set := true
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(stmt_p_return), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !ret_expr_is_set then Pbrt.Decoder.missing_field "ret_expr";
  ({ Frontend_types.ret_expr = v.ret_expr } : Frontend_types.stmt_p_return)
;;
...
let rec decode_stmt_p_while d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(stmt_p_while), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.while_block <- decode_stmt (Pbrt.Decoder.nested d);
      while_block_is_set := true
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(stmt_p_while), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
...
and decode_stmt d =
  let rec loop () =
    let ret : Frontend_types.stmt =
      match Pbrt.Decoder.key d with
      | None -> Pbrt.Decoder.malformed_variant "stmt"
      | Some (1, _) ->
        (Frontend_types.Var_decl (decode_stmt_p_var_decl (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (2, _) ->
        (Frontend_types.Expr_stmt (decode_stmt_p_expr_stmt (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (3, _) ->
        (Frontend_types.Printf (decode_stmt_p_printf (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (4, _) ->
        (Frontend_types.While (decode_stmt_p_while (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (5, _) ->
        (Frontend_types.Block (decode_stmt_p_block (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (6, _) ->
        Pbrt.Decoder.empty_nested d;
      ...
      | Some (10, _) ->
        (Frontend_types.Free (decode_stmt_p_free (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (11, _) ->
        (Frontend_types.Return (decode_stmt_p_return (Pbrt.Decoder.nested d))
          : Frontend_types.stmt)
      | Some (n, payload_kind) ->
        Pbrt.Decoder.skip d payload_kind;
        loop ()
...
and decode_stmt_p_block d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(stmt_p_block), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({ Frontend_types.stmt_list = v.stmt_list } : Frontend_types.stmt_p_block)
...
and decode_stmt_p_if_stmt d =
  ...
  while !continue__ do
    ...
    | Some (2, Pbrt.Bytes) ->
      v.if_stmt <- decode_stmt (Pbrt.Decoder.nested d);
      if_stmt_is_set := true
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(stmt_p_if_stmt), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
      v.else_stmt <- decode_stmt (Pbrt.Decoder.nested d);
      else_stmt_is_set := true
    | Some (3, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(stmt_p_if_stmt), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !else_stmt_is_set then Pbrt.Decoder.missing_field "else_stmt";
  if not !if_stmt_is_set then Pbrt.Decoder.missing_field "if_stmt";
...
let rec decode_class_def d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
      v.name <- Pbrt.Decoder.string d;
      name_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(class_def), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.fields <- decode_type_expr (Pbrt.Decoder.nested d) :: v.fields
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(class_def), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
...
let rec decode_param d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) ->
      v.param_type <- decode_type_expr (Pbrt.Decoder.nested d);
      param_type_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(param), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.param_name <- Pbrt.Decoder.string d;
      param_name_is_set := true
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(param), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !param_name_is_set then Pbrt.Decoder.missing_field "param_name";
  if not !param_type_is_set then Pbrt.Decoder.missing_field "param_type";
  ({ Frontend_types.param_type = v.param_type; Frontend_types.param_name = v.param_name }
...
let rec decode_function_def d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
      v.name <- Pbrt.Decoder.string d;
      name_is_set := true
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(function_def), field(1)" pk
    | Some (2, Pbrt.Bytes) ->
      v.return_t <- decode_type_expr (Pbrt.Decoder.nested d);
      return_t_is_set := true
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(function_def), field(2)" pk
    | Some (3, Pbrt.Bytes) -> v.params <- decode_param (Pbrt.Decoder.nested d) :: v.params
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(function_def), field(3)" pk
    | Some (4, Pbrt.Bytes) ->
      v.body <- decode_stmt (Pbrt.Decoder.nested d);
      body_is_set := true
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(function_def), field(4)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  if not !body_is_set then Pbrt.Decoder.missing_field "body";
  if not !return_t_is_set then Pbrt.Decoder.missing_field "return_t";
  if not !name_is_set then Pbrt.Decoder.missing_field "name";
  ({ Frontend_types.name = v.name
...
let rec decode_program d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.main <- decode_stmt (Pbrt.Decoder.nested d) :: v.main
    ...
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(program), field(2)" pk
    | Some (3, Pbrt.Bytes) ->
      v.function_defs <- decode_function_def (Pbrt.Decoder.nested d) :: v.function_defs
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(program), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
...
let rec encode_expr_p_null_lit (v : Frontend_types.expr_p_null_lit) encoder =
  Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
  Pbrt.Encoder.nested (encode_type_expr v.Frontend_types.null_texpr) encoder;
  ()
;;
...
and encode_expr (v : Frontend_types.expr) encoder =
  ...
  | Frontend_types.Null_lit x ->
    Pbrt.Encoder.key (10, Pbrt.Bytes) encoder;
    Pbrt.Encoder.nested (encode_expr_p_null_lit x) encoder
...
let rec encode_stmt_p_return (v : Frontend_types.stmt_p_return) encoder =
  Pbrt.Encoder.key (1, Pbrt.Bytes) encoder;
  Pbrt.Encoder.nested (encode_expr v.Frontend_types.ret_expr) encoder;
  ()
;;
...
and encode_stmt (v : Frontend_types.stmt) encoder =
  ...
  | Frontend_types.Return x ->
    Pbrt.Encoder.key (11, Pbrt.Bytes) encoder;
    Pbrt.Encoder.nested (encode_stmt_p_return x) encoder
