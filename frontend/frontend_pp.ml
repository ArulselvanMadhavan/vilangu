...
let rec pp_type_expr fmt (v : Frontend_types.type_expr) =
  ...
  | Frontend_types.Int32_ty x ->
    Format.fprintf fmt "@[<hv2>Int32_ty(@,%a)@]" pp_type_expr_p_ranked_type x
;;
...
and pp_expr fmt (v : Frontend_types.expr) =
  ...
  | Frontend_types.If_expr x ->
    Format.fprintf fmt "@[<hv2>If_expr(@,%a)@]" pp_expr_p_if_expr x
  | Frontend_types.Block_expr x ->
    Format.fprintf fmt "@[<hv2>Block_expr(@,%a)@]" pp_expr_p_block x
