...
let rec pp_type_expr fmt (v:Frontend_types.type_expr) =
  ...
  | Frontend_types.Class x -> Format.fprintf fmt "@[<hv2>Class(@,%a)@]" pp_type_expr_p_class x
  | Frontend_types.Pointer x -> Format.fprintf fmt "@[<hv2>Pointer(@,%a)@]" pp_type_expr_p_pointer x
...
let rec pp_expr_p_null_lit fmt (v:Frontend_types.expr_p_null_lit) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "null_texpr" pp_type_expr fmt v.Frontend_types.null_texpr;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_var_p_subscript fmt (v:Frontend_types.var_p_subscript) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "line_no" Pbrt.Pp.pp_int32 fmt v.Frontend_types.line_no;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

and pp_var fmt (v:Frontend_types.var) =
  match v with
  | Frontend_types.Simple x -> Format.fprintf fmt "@[<hv2>Simple(@,%a)@]" pp_var_p_simple x
  | Frontend_types.Subscript x -> Format.fprintf fmt "@[<hv2>Subscript(@,%a)@]" pp_var_p_subscript x
  | Frontend_types.Field x -> Format.fprintf fmt "@[<hv2>Field(@,%a)@]" pp_var_p_field x
  | Frontend_types.Load_var x -> Format.fprintf fmt "@[<hv2>Load_var(@,%a)@]" pp_var_p_load x
...
and pp_var_p_field fmt (v:Frontend_types.var_p_field) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "field_index" Pbrt.Pp.pp_int32 fmt v.Frontend_types.field_index;
    Pbrt.Pp.pp_record_field ~first:false "field_line_no" Pbrt.Pp.pp_int32 fmt v.Frontend_types.field_line_no;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
and pp_expr fmt (v:Frontend_types.expr) =
  ...
  | Frontend_types.Array_creation x -> Format.fprintf fmt "@[<hv2>Array_creation(@,%a)@]" pp_expr_p_array_creation x
  | Frontend_types.Var_exp x -> Format.fprintf fmt "@[<hv2>Var_exp(@,%a)@]" pp_var x
  | Frontend_types.Null_lit x -> Format.fprintf fmt "@[<hv2>Null_lit(@,%a)@]" pp_expr_p_null_lit x
  | Frontend_types.Cast_expr x -> Format.fprintf fmt "@[<hv2>Cast_expr(@,%a)@]" pp_expr_p_cast_expr x
  | Frontend_types.Class_creation x -> Format.fprintf fmt "@[<hv2>Class_creation(@,%a)@]" pp_expr_p_class_creation x
  | Frontend_types.Method_call x -> Format.fprintf fmt "@[<hv2>Method_call(@,%a)@]" pp_expr_p_method_call x
...
and pp_expr_p_binop fmt (v:Frontend_types.expr_p_binop) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "op_line_no" Pbrt.Pp.pp_int32 fmt v.Frontend_types.op_line_no;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
and pp_expr_p_array_creation fmt (v:Frontend_types.expr_p_array_creation) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "creation_exprs" (Pbrt.Pp.pp_list pp_expr) fmt v.Frontend_types.creation_exprs;
    Pbrt.Pp.pp_record_field ~first:false "texpr" pp_type_expr fmt v.Frontend_types.texpr;
    Pbrt.Pp.pp_record_field ~first:false "make_line_no" Pbrt.Pp.pp_int32 fmt v.Frontend_types.make_line_no;
    Pbrt.Pp.pp_record_field ~first:false "arr_cons_idx" Pbrt.Pp.pp_int32 fmt v.Frontend_types.arr_cons_idx;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

and pp_expr_p_cast_expr fmt (v:Frontend_types.expr_p_cast_expr) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "cast_to" pp_type_expr fmt v.Frontend_types.cast_to;
    Pbrt.Pp.pp_record_field ~first:false "expr" pp_expr fmt v.Frontend_types.expr;
    Pbrt.Pp.pp_record_field ~first:false "cast_type" pp_expr_p_cast fmt v.Frontend_types.cast_type;
    Pbrt.Pp.pp_record_field ~first:false "cast_line_no" Pbrt.Pp.pp_int32 fmt v.Frontend_types.cast_line_no;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

and pp_expr_p_class_creation fmt (v:Frontend_types.expr_p_class_creation) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "con_texpr" pp_type_expr fmt v.Frontend_types.con_texpr;
    Pbrt.Pp.pp_record_field ~first:false "con_args" (Pbrt.Pp.pp_list pp_expr) fmt v.Frontend_types.con_args;
    Pbrt.Pp.pp_record_field ~first:false "vtable_index" Pbrt.Pp.pp_int32 fmt v.Frontend_types.vtable_index;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

and pp_expr_p_method_call fmt (v:Frontend_types.expr_p_method_call) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "method_idx" Pbrt.Pp.pp_int32 fmt v.Frontend_types.method_idx;
    Pbrt.Pp.pp_record_field ~first:false "obj_expr" pp_expr fmt v.Frontend_types.obj_expr;
    Pbrt.Pp.pp_record_field ~first:false "method_args" (Pbrt.Pp.pp_list pp_expr) fmt v.Frontend_types.method_args;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_stmt_p_return fmt (v:Frontend_types.stmt_p_return) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "ret_expr" pp_expr fmt v.Frontend_types.ret_expr;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_stmt_p_while fmt (v:Frontend_types.stmt_p_while) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "while_block" pp_stmt fmt v.Frontend_types.while_block;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

and pp_stmt fmt (v:Frontend_types.stmt) =
  match v with
  | Frontend_types.Var_decl x -> Format.fprintf fmt "@[<hv2>Var_decl(@,%a)@]" pp_stmt_p_var_decl x
  | Frontend_types.Expr_stmt x -> Format.fprintf fmt "@[<hv2>Expr_stmt(@,%a)@]" pp_stmt_p_expr_stmt x
  | Frontend_types.Printf x -> Format.fprintf fmt "@[<hv2>Printf(@,%a)@]" pp_stmt_p_printf x
  ...
  | Frontend_types.If_stmt x -> Format.fprintf fmt "@[<hv2>If_stmt(@,%a)@]" pp_stmt_p_if_stmt x
  | Frontend_types.Delete x -> Format.fprintf fmt "@[<hv2>Delete(@,%a)@]" pp_stmt_p_delete x
  | Frontend_types.Free x -> Format.fprintf fmt "@[<hv2>Free(@,%a)@]" pp_stmt_p_free x
  | Frontend_types.Return x -> Format.fprintf fmt "@[<hv2>Return(@,%a)@]" pp_stmt_p_return x

and pp_stmt_p_block fmt (v:Frontend_types.stmt_p_block) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "stmt_list" (Pbrt.Pp.pp_list pp_stmt) fmt v.Frontend_types.stmt_list;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
and pp_stmt_p_if_stmt fmt (v:Frontend_types.stmt_p_if_stmt) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "else_stmt" pp_stmt fmt v.Frontend_types.else_stmt;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_class_def fmt (v:Frontend_types.class_def) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "name" Pbrt.Pp.pp_string fmt v.Frontend_types.name;
    Pbrt.Pp.pp_record_field ~first:false "fields" (Pbrt.Pp.pp_list pp_type_expr) fmt v.Frontend_types.fields;
    Pbrt.Pp.pp_record_field ~first:false "base_class_name" Pbrt.Pp.pp_string fmt v.Frontend_types.base_class_name;
    Pbrt.Pp.pp_record_field ~first:false "vtable" (Pbrt.Pp.pp_list Pbrt.Pp.pp_string) fmt v.Frontend_types.vtable;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

let rec pp_param fmt (v:Frontend_types.param) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "param_type" pp_type_expr fmt v.Frontend_types.param_type;
    Pbrt.Pp.pp_record_field ~first:false "param_name" Pbrt.Pp.pp_string fmt v.Frontend_types.param_name;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_function_def fmt (v:Frontend_types.function_def) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "name" Pbrt.Pp.pp_string fmt v.Frontend_types.name;
    Pbrt.Pp.pp_record_field ~first:false "return_t" pp_type_expr fmt v.Frontend_types.return_t;
    Pbrt.Pp.pp_record_field ~first:false "params" (Pbrt.Pp.pp_list pp_param) fmt v.Frontend_types.params;
...
let rec pp_program fmt (v:Frontend_types.program) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "main" (Pbrt.Pp.pp_list pp_stmt) fmt v.Frontend_types.main;
    Pbrt.Pp.pp_record_field ~first:false "classdefs" (Pbrt.Pp.pp_list pp_class_def) fmt v.Frontend_types.classdefs;
    Pbrt.Pp.pp_record_field ~first:false "function_defs" (Pbrt.Pp.pp_list pp_function_def) fmt v.Frontend_types.function_defs;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
